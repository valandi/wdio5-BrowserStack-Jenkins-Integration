...
class EyesRunner {
    ...
    async getAllTestResults(throwErr = true) {
        ...
        const deleteTest = (options) => this._spec.deleteTest(Object.assign(Object.assign({}, options), { serverUrl: eyes.configuration.serverUrl, apiKey: eyes.configuration.apiKey, proxy: eyes.configuration.proxy }));
        try {
            const results = await this._manager.closeAllEyes({ throwErr });
            return new TestResultsSummary_1.TestResultsSummaryData(results.map(result => new TestResults_1.TestResultsData(result, deleteTest)));
        }
        catch (err) {
